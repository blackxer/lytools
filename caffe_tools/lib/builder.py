...
class Net:
    ...
    def __init__(self, pt=None, phase=None):#, name="network"
    ...
    def set_bottom(self, layer_name, newbottom):
        for i in self.layer[layer_name]:
            ...
            ## my adding
            if layer_name.endswith('loss') and "label" in i.bottom:
                i.bottom.remove('label')
                i.bottom.extend(['label'])
            ## my adding
    ...
    def insert_input(self, input="data", dim=[1, 3, 256, 512]):
        self.net.input.extend([input])
        # self.net.input_shape.dim.extend(dim)
        input_shape = self.net.input_shape.add()
        for i in range(len(dim)):
            input_shape.dim.extend([dim[i]])

    def ch_bottom(self, layer_name, newbottom, oldbottom):
        '''
        if inline layer, also change top
        '''
        if not self.rm_bottom(layer_name, oldbottom):       ####如果oldbottom是layer_name的bottom，删除oldbottom,返回true,不是则返回false
            return False
        self.set_bottom(layer_name, newbottom)              ####设置layer_name的bottom为newbottom，即r
    ...
    def ch_top(self, layer_name, newtop, oldtop):
        if not self.rm_top(layer_name, oldtop):            #####如果oldtop是layer_name的top，则删除oldbtop,返回true,如果不是返回false
            return False
        self.set_top(layer_name, newtop)                  ####设置layer_name的top为newtop，即r
        return True

    ## 删除layer_name层的所有param参数，并返回param的个数
    def rm_param(self, layer_name):
        len = 0
        for i in self.layer[layer_name]:
            if i.param:
                while i.param:
                    for param in i.param:
                        len += 1
                        i.param.remove(param)
            else:
                return 0
        return len

    ## 增加param参数
    def add_param(self, layer_name, lr_mult, decay_mult):
        for i in self.layer[layer_name]:
            param = i.param.add()
    ...
    def set_use_global_stats(self, bn_name, stats=True):
        layer_type = self.layer_type(bn_name)
        assert layer_type == "BatchNorm"
        for i in self.layer[bn_name]:
            i.batch_norm_param.use_global_stats = stats

    def set_propagate_down(self, layer_name, stats=False):
        for layer in self.layer[layer_name]:
            pd_len = 0
            for pd in layer.propagate_down:
                pd_len += 1
            bottoms = self.layer_bottom(layer_name)
            if isinstance(bottoms, str):
                btm_len = 1
            else:
                btm_len = len(bottoms)
            if pd_len == 0:
                for i in range (btm_len):
                    layer.propagate_down.extend([stats])
            else:
                while layer.propagate_down:
                    for pd in layer.propagate_down:
                        layer.propagate_down.remove(pd)
                for i in range (btm_len):
                    layer.propagate_down.extend([stats])
    ...
    def merge_net(self, new_nets=None, bottomname=None, afterinline=False):
        # newlayer = self.net.layer[-1]
        # self.net.layer.remove(newlayer)
        appendix_self = []
            ...
            while tmp != bottomname:
                if afterinline:
                    try:
                        ...
                        pass
                behind = self.net.layer[-1]
                appendix_self.append(behind)
                self.net.layer.remove(behind)
                tmp = self.net.layer[-1].name
        for new_net in new_nets:
            appendix_other = []
            while len(new_net.layer):
                behind = new_net.net.layer[-1]
                appendix_other.append(behind)
                new_net.net.layer.remove(behind)
            self.net.layer.extend(appendix_other[::-1])
        self.net.layer.extend(appendix_self[::-1])


    def rm_layer(self, name, inplace=False):
        if not inplace:
            bottoms = self.layer[name][0].bottom
            # assert len(bottom) == 1
            # bottom = bottom[0]
    ...
    def setup(self, name, layer_type, bottom=[], top=[], inplace=False):
        ...
        # if layer_type == 'EvalDetection':

        self.this = new_layer
        if not inplace:
            self.cur = new_layer

        if self.bottom is not None and not new_layer.type.endswith('Data'):   #new_layer.type not in ['DummyData', 'Data']
            bottom_name = [self.bottom.name]
            if len(bottom) == 0:
                bottom = bottom_name
            for i in bottom:
                new_layer.bottom.extend([i])
    ...
    def type2names(self, layer_type="Convolution"):
        names = []
        for i in self.layer:
            if self.layer_type(i) in layer_type:
                names.append(i)
    ...
    def transform_param(self, mean_value=128, scale=.0078125, mirror=1, crop_size=None, mean_file_size=None, phase=None):
    ...
    def weight_filler(self, filler=filler.xavier, std=None):      #原默认msra
        """xavier"""
    ...
    def ReLU(self, name=None, inplace=True):
        ...
        # reluname = self.suffix('relu', name)
        self.setup(name, 'ReLU', inplace=inplace)
        relu_param = self.this.relu_param
    ...
    def BatchNorm(self, name=None, inplace=True, eps=1e-5):
        ...
        self.setup(name, 'BatchNorm', inplace=inplace)
        # self.include()
    ...
    def Scale(self, name=None, inplace=True, bias=False, suf=True):
        # sname = self.suffix('scale', name)
        self.setup(name, 'Scale', inplace=inplace)
        if bias:
            self.param(0,0)
            self.param(0,0)
            NotImplementedError
    ...
    def Data(self, source, type='Data', top=['data', 'label'], name="data", phase=None, **kwargs):
        self.setup(name, type, top=top)
    ...
    def  Convolution(self,
        ...
        if lr_mult is not False and decay_mult is not False:
            lr = lr_mult
            decay = decay_mult
            self.param(lr_mult=lr, decay_mult=decay)
    ...
    def EuclideanLoss(self, name, bottom, loss_weight=-1, propagate_down=True, **kwargs):
        ...
        if not propagate_down :
            for i in range(len(bottom)):
                self.this.propagate_down.extend([propagate_down])
        for val_name, val in kwargs.items():
            if val_name=='phase':
                self.include(val)

    # def EuclideanLoss(self, name, bottom, **kwargs):                 # loss_weight=1,
    #     self.setup(name, 'EuclideanLoss', bottom=[bottom], top=[name], inplace=True)
        
    def EvalDetection(self,
                    name,
                    bottom=[],
                    top=None,
                    propagate_down=True,
        ...
        self.setup(name, 'EvalDetection', bottom=bottom, top=top , **kwargs)  #propagate_down=False
        if not propagate_down:
            for i in range(len(bottom)):
                self.this.propagate_down.extend([propagate_down])

        eval_detection_param = self.this.eval_detection_param
        eval_detection_param.side_w = side_w
        eval_detection_param.side_h = side_h
        eval_detection_param.num_class = num_class
        biases2 = [2.411610, 2.205474, 3.548339, 2.999060]
        biases5 = [1.08, 1.19, 3.42, 4.41, 6.63, 11.38, 9.42, 5.11, 16.62, 10.52]
        if num_object != 5:
            assert num_object == 2
            eval_detection_param.num_object = 2
            eval_detection_param.biases.extend(biases2)
        else:
            eval_detection_param.num_object = 5
            eval_detection_param.biases.extend(biases5)
        eval_detection_param.threshold = threshold
        eval_detection_param.nms = nms
        eval_detection_param.max_objnum = int(max_objnum)
        # if phase is not None:
        #     self.include(phase)
    ...
    def RegionLoss(self,
        ...
        self.setup(name, 'RegionLoss', bottom=bottom, top=top, **kwargs)  #propagate_down=False
        if not propagate_down:
            for i in range(len(bottom)):
                self.this.propagate_down.extend([propagate_down])
        if loss_weight != -1:
        ...
        if phase == caffe.TRAIN:
            self.include('TRAIN')
        region_loss_param = self.this.region_loss_param
        # if side_w != 12:
        region_loss_param.side_w = side_w
        # if side_h != 12:
        region_loss_param.side_h = side_h
        # if num_class != 1:
        region_loss_param.num_class = num_class
        region_loss_param.coords = coords
        biases2 = [2.411610, 2.205474, 3.548339, 2.999060]
        biases5 = [1.08, 1.19, 3.42, 4.41, 6.63, 11.38, 9.42, 5.11, 16.62, 10.52]
        if num != 5:
            assert num == 2
            region_loss_param.num = 2
            region_loss_param.biases.extend(biases2)
        else:
            region_loss_param.num = 5
            region_loss_param.biases.extend(biases5)
        region_loss_param.softmax = softmax
        region_loss_param.jitter = jitter
        region_loss_param.rescore = rescore
        region_loss_param.object_scale = object_scale
        region_loss_param.noobject_scale = noobject_scale
        region_loss_param.class_scale = class_scale
        region_loss_param.coord_scale = coord_scale
        region_loss_param.absolute = absolute
        region_loss_param.thresh = thresh
        region_loss_param.random = random
    ...
    def Pooling(self, name, pool='MAX', global_pooling=False, pad=0, stride=1, kernel_size=3):
        """MAX AVE """
